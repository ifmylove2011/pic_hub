...
public class ImageLoader4 {
	...
	static class MainHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case MESSAGE_POST_RESULT:
				LogUtils.i("setBitmaps");
				LoaderResult result = (LoaderResult) msg.obj;
				RootView rv = result.rootView;
				String[] uris = (String[]) rv.getTag(TAG_KEY_URI);
				if (Arrays.equals(uris, result.uris)) {
					result.rootView.setBitmaps(result.bitmaps);
				} else {
					LogUtils.w("uri changed");
				}
				break;
	...
	private ImageLoader4(Context context) {
	 ...
	 */
	public static ImageLoader4 build(Context context) {
		return new ImageLoader4(context);
	}
	...
	public void bindBitmap(final String[] uris, final RootView rv, boolean isSquare) {
		mIsSquare = isSquare;
		bindBitmap(uris, rv, ViewUtils.getDefaultSize() / 4, ViewUtils.getDefaultSize() / 4, isSquare);
	}
	 ...
	 *            要求高度
	 */
	public void bindBitmap(final String[] uris, final RootView rootView, final int reqWidth, final int reqHeight,
			boolean isSquare) {
		mIsSquare = isSquare;
		rootView.setTag(TAG_KEY_URI, uris);
		...
		Runnable loadBitmapTask = new Runnable() {
			...
			public void run() {
				...
				LoaderResult result = new LoaderResult(uris, bitmaps, rootView);
	...
	private static class LoaderResult {
		...
		public LoaderResult(String[] uris, Bitmap[] bitmaps, RootView rootView) {
			super();
			this.uris = uris;
